...
public class UserProcess {
	...
	private int handleWrite(int i, int addr, int size){
		...
		if ((numPages*pageSize)<addr|| (numPages*pageSize)<(addr+size))
			return -1;
		...
		int numBytesWritten = writable.write(author, 0, tmpLength);

		if(numBytesWritten == -1|| numBytesWritten != size) {
			return -1;
		}
	...
	private int handleRead(int i, int addr, int size){
		...
		//if() physical address is numpages * pagesize, if address or address plus size is bigger return -1
		...
		int numBytesNewlyWritten = writeVirtualMemory(addr, buffer, 0, numBytesRead);
		if( numBytesNewlyWritten != numBytesRead) {
			return -1;
		}
		return numBytesNewlyWritten;
